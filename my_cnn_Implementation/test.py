...
tr_inputs, tr_labels = tr_data[0], tr_data[1]
n = len(tr_inputs)
training_inputs = [tr_inputs[k:k+100] for k in range(0,n,100)]
training_labels = [tr_labels[k:k+100] for k in range(0,n,100)]
no_of_mini_batch = len(training_inputs)

test_inputs, test_labels = te_data[0], te_data[1]
n1 = len(test_inputs)
test_inputs = [test_inputs[k:k+100] for k in range(0,n1,100)]
test_labels = [test_labels[k:k+100] for k in range(0,n1,100)]
no_of_mini_test = len(test_inputs)
...
class ConvNet(object):
    ...
    def conv_layers(self):
        ...
        label = tf.placeholder(tf.int32,name='keep_prob')
        ...
        flatten = tf.reshape(output,[-1,shape])
